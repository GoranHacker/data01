...
# Procesar y mostrar los datos_precios para cada especie
for (especie in orden_especies) {
  datos_precios_especie <- datos_precios_30_dias %>%
    filter(ESPECIE == especie) %>%
    summarise(
  ...
  if(nrow(datos_precios_especie) > 0) {
    cat(paste("\n\n###", especie, "\n"))
    datos_precios_tabla <- rbind(
      c("KG", datos_precios_especie$Media_KG, datos_precios_especie$Minimo_KG, datos_precios_especie$Maximo_KG),
      c("CAJA", datos_precios_especie$Media_CAJA, datos_precios_especie$Minimo_CAJA, datos_precios_especie$Maximo_CAJA),
      c("MALLA", datos_precios_especie$Media_MALLA, datos_precios_especie$Minimo_MALLA, datos_precios_especie$Maximo_MALLA)
    )
    colnames(datos_precios_tabla) <- c("Tipo", "Media", "Mínimo", "Máximo")
    print(
      knitr::kable(datos_precios_tabla, "html") %>%
...
ggplot(data = merluza_comun_datos_precios, aes(x = FECHA, y = Precio_Promedio)) +
  geom_line(colour = "red", size = 1) +  # Línea roja
  geom_point(colour = "black", size = 3, shape = 21, fill = "pink") +  # Puntos en los datos_precios
