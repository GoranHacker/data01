...
condiciones_adversas <- any(datos_filtrados$windspeed_kph > 30 | datos_filtrados$swell_period_secs > 20 | datos_filtrados$wave_height_m > 4 | datos_filtrados$Direccion_Viento %in% c("Norte", "Noreste", "Noroeste"))

if (!condiciones_adversas) {
  cat("<p style='font-weight: bold; color: red;'>Las condiciones meteorológicas podrÃ­an ser positivas para la pesca.</p>")
} else {
  cat("<p style='font-weight: bold; color: red;'>Las condiciones meteorológicas son malas para la pesca.</p>")
}
```
...
# Conexión a la base de datos mediante DSN
sernapesca <- odbcConnect(dsn = "Rquimera")

# Cambiar el contexto a la base de datos Fip
invisible(sqlQuery(sernapesca, "USE Fip"))

# Consulta para obtener los campos relevantes
consulta <- "
SELECT
  Cd_Oficina,
  Nr_Cometido,
...
FROM
  Fip_Cometidos
WHERE
  YEAR(Fc_Inicio) = 2024 AND
  Cd_Oficina IN (36, 40, 41)
"

# Ejecutar la consulta
resultado <- sqlQuery(sernapesca, consulta)

# Procesar los datos en R
resultado <- resultado %>%
  mutate(
    Año_Cometido = format(as.Date(Fc_Inicio), "%Y"),
    Mes_Cometido = format(as.Date(Fc_Inicio), "%m"),
    Fecha_Cometido = format(as.Date(Fc_Inicio), "%d-%m-%Y"),
    Numero_Cometido = paste(Cd_Oficina, Nr_Cometido, sep = "-"),
    Oficina = case_when(
      Cd_Oficina == 36 ~ "CONSTITUCION",
      Cd_Oficina == 40 ~ "PELLUHUE",
      Cd_Oficina == 41 ~ "LICANTEN",
      TRUE ~ NA_character_
    )
  )

# Filtrar datos para los últimos 30 días, 7 días y ayer
today <- Sys.Date()
last_30_days <- resultado %>% filter(as.Date(Fc_Inicio) >= (today - 30))
last_7_days <- resultado %>% filter(as.Date(Fc_Inicio) >= (today - 7))
yesterday <- resultado %>% filter(as.Date(Fc_Inicio) == (today - 1))

# Resumir los datos
summary_last_30_days <- last_30_days %>% group_by(Oficina) %>% summarise(Cometidos = n())
summary_last_7_days <- last_7_days %>% group_by(Oficina) %>% summarise(Cometidos = n())
summary_yesterday <- yesterday %>% group_by(Oficina) %>% summarise(Cometidos = n())

# Combinar los resúmenes en una sola tabla
summary_table <- summary_last_30_days %>%
  rename(Cometidos_30_Dias = Cometidos) %>%
  left_join(summary_last_7_days %>% rename(Cometidos_7_Dias = Cometidos), by = "Oficina") %>%
  left_join(summary_yesterday %>% rename(Cometidos_Ayer = Cometidos), by = "Oficina")
...
# Conexión a la base de datos SIGCUE mediante DSN
...
resultado_sigcue_detalle <- resultado_sigcue_detalle %>%
  ...
  select(Nr_declaracion, Especie, Desembarque)
...
WHERE
  YEAR(Fc_Llegada) = YEAR(GETDATE()) AND
...
# Filtrar los datos del día anterior a hoy
yesterday <- Sys.Date() - 1
datos_ayer <- resultado_combinado %>%
  filter(Fecha_Desembarque == yesterday)

# Resumir los datos por especie y caleta para desembarque
resumen_desembarque <- datos_ayer %>%
...
# Resumir los datos por especie y caleta para declaraciones
resumen_declaraciones <- datos_ayer %>%
  group_by(Caleta, Especie) %>%
  summarise(
...
# Unir los resúmenes en una sola tabla
resumen_final <- resumen_declaraciones %>%
  left_join(resumen_desembarque, by = c("Caleta", "Especie"))
