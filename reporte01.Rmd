...
author: "Grupo Estrategico Macrozonal Merluza ComÃºn GEMM"
date: "`r Sys.Date()`"
output:
  html_document:
    self_contained: true
...
<p>Este documento, desarrollado en conjunto por la Unidad de Inteligencia y las Direcciones Regionales en el marco del Programa GEMM, ofrece un anÃ¡lisis actualizado de las actividades de pesca en caletas del Maule y los precios en el TPM. El objetivo es facilitar la toma de decisiones estratÃ©gicas y la activaciÃ³n de fiscalizaciones efectivas.</p>
<p><strong>Fuentes de Datos:</strong></p>
<ul>
  <li><strong>Datos de precios y zarpes:</strong> Recopilados diariamente de hojas de cÃ¡lculo en lÃ­nea gestionadas por las regiones.</li>
  <li><strong>InformaciÃ³n meteorolÃ³gica:</strong> Es obtenida de APIs que proporcionan pronÃ³sticos actualizados cada hora, para complementar los anÃ¡lisis con las condiciones climÃ¡ticas que impactan la pesca.</li>
</ul>
<p><strong>AutoactualizaciÃ³n:</strong> El informe se actualiza automÃ¡ticamente cada hora mediante cron jobs, garantizando la relevancia de la informaciÃ³n y demostrando el potencial de la tecnologÃ­a para optimizar la gestiÃ³n y monitoreo en tiempo real.</p>
<p>Este reporte es parte de una iniciativa en fase inicial y se espera que evolucione con el aporte de diversos actores de SERNAPESCA, refinando y validando continuamente los anÃ¡lisis presentados.</p>
</div>
...
<div style="font-size:24px; font-weight:bold;">
  Datos climÃ¡ticos de las comunas del Maule
</div>
...
# Define la fecha actual antes de cualquier operaciÃ³n que la utilice
...
datos_filtrados <- datos_clima %>%
  ...
  mutate(
    Direccion_Viento = case_when(
      wind_direction < 22.5 | wind_direction >= 337.5 ~ "Norte",
      wind_direction >= 22.5 & wind_direction < 67.5 ~ "Noreste",
      wind_direction >= 67.5 & wind_direction < 112.5 ~ "Este",
      wind_direction >= 112.5 & wind_direction < 157.5 ~ "Sureste",
      wind_direction >= 157.5 & wind_direction < 202.5 ~ "Sur",
      wind_direction >= 202.5 & wind_direction < 247.5 ~ "Suroeste",
      wind_direction >= 247.5 & wind_direction < 292.5 ~ "Oeste",
      wind_direction >= 292.5 & wind_direction < 337.5 ~ "Noroeste",
      TRUE ~ "Desconocido"
    ),
...
# EvaluaciÃ³n de las condiciones meteorolÃ³gicas
...
if (!condiciones_adversas) {
  cat("<p style='font-weight: bold; color: red;'>Las condiciones meteorolÃ³gicas podrÃ­an ser positivas para la pesca.</p>")
} else {
  cat("<p>Las condiciones meteorolÃ³gicas son malas para la pesca.</p>")
}
```

<div style="font-size:24px; font-weight:bold;">
  InformaciÃ³n de precios de recursos en el TPM
</div>
...
if(nrow(datos_precios_ultima_fecha_precio) > 0) {
  ...
  print("No hay datos_precios disponibles para la Ã¼ltima fecha.")
}
...
# CondiciÃ³n de alerta especÃ­fica para Merluza ComÃºn
precio_umbral_merluza_comun <- 30000

# Filtrar los datos solo para Merluza ComÃºn
...
# Verificar condiciÃ³n de riesgo para Merluza ComÃºn
...
# EvaluaciÃ³n combinada de riesgo de precio y zarpe
...
if (Riesgo_Precio == 1 && Riesgo_Zarpe == 0) {
  nivel_alerta <- "Media"
  mensaje_alerta <- "Alerta Media: Precio de Merluza ComÃºn bajo y sin riesgos significativos de zarpe."
  color_alerta <- "orange"
} else if (Riesgo_Precio == 0 && Riesgo_Zarpe >= 1) {
  nivel_alerta <- "Media"
  mensaje_alerta <- "Alerta Media: Zarpes significativos, sin riesgo por el precio de la Merluza ComÃºn."
  color_alerta <- "orange"
} else if (Riesgo_Precio == 1 && Riesgo_Zarpe > 0) {
  nivel_alerta <- "Alto"
  mensaje_alerta <- "Riesgo Alto: Riesgos crÃ­ticos tanto por precio como por zarpe para Merluza ComÃºn."
  color_alerta <- "red"
} else {
  nivel_alerta <- "Bajo"
  mensaje_alerta <- "Riesgo Bajo: No se detectan condiciones de riesgo significativas para la Merluza ComÃºn hoy."
  color_alerta <- "green"
}

# Mostrar el resultado de la evaluaciÃ³n de riesgo
...
<div style="font-size:24px; font-weight:bold;">
  GrÃ¡fico Precio promedio merluza comÃºn en los ultimos 30 dÃ­as (TPM)
</div>
