...
drive_auth(path = "key.json")### esto cambiar a donde quede el archivo key.json


orden_especies <- c("MERLUZA COMUN", "MERLUZA AUSTRAL", "REINETA", "CONGRIO DORADO", "JUREL", "LOCOS", "CHORITO", "ALMEJA")

folder_id <- "1Qq6qK9TAYm7QfycULcZxYKaFX0oAEYOa"
  file_name <- "forecast"
  ...
  file_info <- files[files$name == file_name, ]
  
  # Download the file
  drive_download(as_id(file_info$id), path = paste0(tempdir(), "/", file_info$name), overwrite = TRUE)
datos_clima <- read.csv(paste0(tempdir(), "/", file_info$name))
...
folder_id <- "18JAc_4N11qkGmk3nimk7RnKSAiQbBDex"
file_name <- "resumen_gesfip.csv"

# Listar archivos en la carpeta y obtener el ID del archivo específico
files_in_folder <- drive_ls(path = as_id(folder_id))
file <- files_in_folder %>% filter(name == file_name)

# Descargar el archivo temporalmente
drive_download(as_id(file$id), path = file_name, overwrite = TRUE)

# Leer el archivo
summary_table <- read_csv(file_name)
...
folder_id <- "18JAc_4N11qkGmk3nimk7RnKSAiQbBDex"
file_name <- "resumen_desembarques_final.csv"
...
# Leer el archivo CSV
resumen_final <- read_csv(file_name)
